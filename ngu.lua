...
notis.new("<Color=White>TienVinh HUB MAIN<Color=/>"):Display()
notis.new("<Color=Blue>Founder: t_vinh<Color=/>"):Display() 
...
local TienVinhHub = loadstring(game:HttpGet((MainLoader)))()
...
function Notify(G, H, I)
    ...
    TienVinhHub:MakeNotification({Name = G, Content = H, Image = "rbxassetid://15214658898", Time = I})
end
...
function CheckSeaBeastTrial()
    if not WS.Map:FindFirstChild("FishmanTrial") then
        chodienspamxienchetcuchungmay = nil
        return nil
    end
...
function InstantChooseGear()
    ...
    if v14 and v14.HadPoint then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "NtienVinh Hub", Text = "You Hadn't Gear", Duration = 30})
        task.wait(30)
    end
end
spawn(function()
            ...
            if TargetedPlayer or getLowestLevelPlayer() then
                ...
                repeat
                    ...
                    if TpCFrame and (TpCFrame.Position - LP.Character.HumanoidRootPart.Position).Magnitude < 300 and not PG.Main.PvpDisabled.Visible then
                        EClick()
                        chodienspamxienchetcuchungmay = true
                        AimbotPos = TpCFrame
                        SkillAim = true
                    else
                        chodienspamxienchetcuchungmay = nil
                ...
                chodienspamxienchetcuchungmay = false
                SkillAim = false
                AimbotPos = nil
                SeWReal = SeWFake
                TPS = false
...
spawn(function()
    while task.wait() do
        if chodienspamxienchetcuchungmay then
            EBuso()
...
function GetQuestV3()
    ...
    if v33000 == 0 then
        ...
        wait(.1)
        Notify("TienVinh Hub", "Claimed Quest V3", 10)
    elseif v33000 == -1 then
        Notify("TienVinh Hub", "You Not Have V2")
    end
end
...
local Window = TienVinhHub:MakeWindow({Name = "NTienVinh Hub", HidePremium = false, SaveConfig = false, ConfigFolder = "NTienVinh Hub Main"})
...
A:AddDropdown({Name = "Select Skill [Click Skill Enable]", Default = "", Options = skill, Callback = function(vSelectSkills)
    ...
    Notify("TienVinh Hub", "Skill " .. tostring(vSelectSkills) .. ": " .. tostring(SkillSelected[vSelectSkills]) .. "")
end    
})
                ...
                if string.find(CheckR, "V1") then
                    ...
                    elseif RS.Remotes.CommF_:InvokeServer("BartiloQuestProgress", "Bartilo") == 1 then
                        ...
                        if Jeremy then
                            ...
                            Notify("TienVinh Hub", "Not Found Boss, Start Hop", 10)
                            HopServer()
                        end
                        ...
                        if LP.Backpack:FindFirstChild("Flower 1") and LP.Backpack:FindFirstChild("Flower 2") and LP.Backpack:FindFirstChild("Flower 3") then
                            ...
                            if not LP.Backpack:FindFirstChild("Flower 1") and not LP.Character:FindFirstChild("Flower 1")then
                                if WS.Flower1.Transparency ~= 1 then
                                    Notify("TienVinh Hub", "Collecting Flower 1", 10)
                                    ...
                                    if game.Lighting.ClockTime > 3 and game.Lighting.ClockTime < 16 then
                                        Notify("TienVinh Hub", "Hop Night", 10)
                                        HopServer()
                                    end
                                end
                            elseif not LP.Backpack:FindFirstChild("Flower 2") and not LP.Character:FindFirstChild("Flower 2") then
                                if WS.Flower2.Transparency ~= 1 then
                                    Notify("TienVinh Hub", "Collecting Flower 2", 10)
                                end
                            ...
                            elseif not LP.Backpack:FindFirstChild("Flower 3") and not LP.Character:FindFirstChild("Flower 3") then
                                Notify("TienVinh Hub", "Collecting Flower 3", 10)
                    ...
                    if CheckR == "Human V2" then
                        ...
                        Notify("TienVinh Hub", sk2, 15)
                        if #sk < 3 and CheckVerRace() == "Human V2" then
                            Notify("TienVinh Hub", "Hop Server Have 3 Boss", 15)
                            task.wait(1)
                            HopServer()
                        end
                        if #sk >= 3 then
                            for aq, cg in pairs(BossQuest) do
                                ...
                                if cg and CheckVerRace() == "Human V2" then
                                    Notify("TienVinh Hub", "Start Killing " .. aq .. " Boss To Up Human V3")
                    ...
                    elseif CheckR == "Skypiea V2" then
                        ...
                        if #SkypieaPlayers > 0 then
                            ...
                            Notify("TienVinh Hub", "Not Found Player, Start Hop", 15)
                            HopServer()
                        end
                    ...
                    elseif CheckR == "Mink V2" then
                        ...
                        if ch and CheckR ~= "Mink V3" then
                            ...
                            Notify("NtienVinh Hub", "Remaning " .. 30 - CountChest .. " Chest", 7.5)
                            if CountChest >= 30 then
                                UpV3NoTween()
                                Notify("TienVinh Hub", "Race Status: " .. tostring(RS.Remotes.CommF_:InvokeServer("Wenlocktoad", "info")))
                            end
                        elseif ch == nil then
                            repeat wait()
                                local ch = ChestNearF()
